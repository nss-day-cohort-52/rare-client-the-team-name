        ...
        <section className="message is-info m-5">
            ...
            <section className="message-body">
                ...
                <div>Tagged {post.tags?.map(t => t.label)}</div>
                    {userFucntion() ? <Link to={`/my-posts/editpost/${post.id}`}><button className="button mr-3 my-3">Edit Post</button></Link> : "" }
                    {userFucntion() ? <button className="button mr-3 my-3" onClick={() => {
                        deletePost(post.id)
                        .then(setPost)
                    }}>Delete Post</button> : "" }
